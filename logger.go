...
func (r *Info) Output() string {
	msg := fmt.Sprintf(r.format /*r.Id, r.Time, r.FileName, r.LineNum,*/, r.Level, r.Message)
	return msg
}
...
// to os.Stderr
func NewLogger(path string, prog string) (*Logger, error) {
	initColors()
	...
	newWorker := NewWorker("", log.Ldate|log.Ltime|log.Lshortfile, 1, file)
	return &Logger{Module: prog, worker: newWorker}, nil
}

// The log commnand is the function available to user to log message, level specifies
